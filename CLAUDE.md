...
3. **Memory (project-local only — see `feedback-memory-location` memory in `memory/`)** — update the project-local `memory/` folder in this repo (untracked in git). Do not write to the harness's global auto-memory folder outside the project. Auto-load at session start is configured via `autoMemoryDirectory`/`autoMemoryEnabled` in `.claude/settings.local.json` (gitignored, machine-local) pointing at this repo's `memory/` folder — so local-only storage and auto-load are both active. Update the relevant `project_*` memory file with current status; add a new `feedback_*` memory only if the user gave new durable guidance this session. Keep `memory/MEMORY.md` as a short index, not a content dump.
