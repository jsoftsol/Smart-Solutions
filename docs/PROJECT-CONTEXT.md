...
```
2026-08-11 — Git hygiene: repo had no root-level `.gitignore`, so the project-local `memory/` folder (untracked by convention only) was one `git add -A` away from being committed. Added a root `.gitignore` excluding `memory/`, committed (`4a920c4`), and pushed. No source changes. `dotnet build` clean (0 errors); `dotnet test` could not be re-verified this pass — blocked locally by a Windows Application Control policy on the freshly-built test DLL (environment issue, not a code regression); last confirmed 35/35 passing in the prior session.
